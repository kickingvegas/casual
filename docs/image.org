...
Casual Image (library: ~casual-image~) is a user interface for Image Mode ([[info:emacs#Image Mode]]).
...
#+VINDEX: casual-image-init

If Casual is installed using ~package-install~, then you can use ~casual-init~ to setup support for Image.

By default, ~casual-init~ will setup support for Image via the hook function ~casual-image-init~ which is included in the customizable variable ~casual-init-hook~.

Use the command ~customize-variable~ on ~casual-init-hook~ to control inclusion of ~casual-image-init~ via a checkbox interface.

Refer to the Simplified Install ([[#install][Install]]) section for more detail on customizing ~casual-init~.

Casual makes opinionated decisions on the keybindings used in its menus. Such opinions are extended from said menus to the keymap(s) of a particular mode. For Image, this can be controlled via the customizable variable ~casual-image-add-extra-keybindings~ (default ~t~).
...
The primary interface for Casual Image is the Transient menu ~casual-image-tmenu~. It is autoloaded ([[info:elisp#Autoload]]) so if Casual is installed via ~package-install~ ([[info:emacs#Package Installation]]) then ~casual-image-tmenu~ can be invoked via ~execute-extended-command~ ({{{kbd(M-x)}}}) without need for modifying your Emacs initialization file.
...
#+begin_src elisp :lexical no
  (require 'casual-image) ; load all dependencies including `image'
  (keymap-set image-mode-map "C-o" #'casual-image-tmenu)
#+end_src

If lazy loading of the ~image~ package is desired then try using ~eval-after-load~:
