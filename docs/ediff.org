...
Casual Ediff (library: ~casual-ediff~) is a user interface for Ediff ([[info:ediff#Top][Ediff]]), a visual interface for the Unix diff and patch utilities. Casual Ediff strives to improve the usability of Ediff by simplifying the following workflows:
...
The primary interface for Casual Ediff is the Transient menu ~casual-ediff-tmenu~. It is autoloaded ([[info:elisp#Autoload]]) so if Casual is installed via ~package-install~ ([[info:emacs#Package Installation]]) then ~casual-ediff-tmenu~ can be invoked via ~execute-extended-command~ ({{{kbd(M-x)}}}) without need for modifying your Emacs initialization file.
