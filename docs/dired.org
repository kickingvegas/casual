...
#+VINDEX: casual-dired-init

If Casual is installed using ~package-install~, then you can use ~casual-init~ to setup support for Dired.

By default, ~casual-init~ will setup support for Dired via the hook function ~casual-dired-init~ which is included in the customizable variable ~casual-init-hook~.

Use the command ~customize-variable~ on ~casual-init-hook~ to control inclusion of ~casual-dired-init~ via a checkbox interface.

Refer to the Simplified Install ([[#install][Install]]) section for more detail on customizing ~casual-init~.

Casual makes opinionated decisions on the keybindings used in its menus. Such opinions are extended from said menus to the keymap(s) of a particular mode. For Dired, this can be controlled via the customizable variable ~casual-dired-add-extra-keybindings~ (default ~t~).
...
#+begin_src elisp :lexical no
  (require 'casual-dired) ; load all dependencies including `dired'
  (keymap-set dired-mode-map "C-o" #'casual-dired-tmenu)
#+end_src

If lazy loading of the ~dired~ package is desired then try using ~eval-after-load~:
...
#+TEXINFO: @subheading Additional Bindings

It is convenient to have both the sort-by (~casual-dired-sort-by-tmenu~) and search & replace (~casual-dired-search-replace-tmenu~) menus bound to ~s~ and ~/~ respectively.
