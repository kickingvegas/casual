...
Given installation via ~package-install~ ([[info:emacs#Package Installation]]), most Casual Transient menus are autoloaded ([[info:elisp#Autoload]]) so they can be invoked without need to modify an Emacs initialization file.

That said, Casual is designed with the intent of using a common keybindings across multiple modes to lower cognitive load. Setting these keybindings requires modification of your Emacs initialization file. These keybindings can be set to user preference. For the purpose of documentation, this document uses the following keybindings:

- Primary binding: {{{kbd(C-o)}}}
- Secondary binding: {{{kbd(M-m)}}}

Casual is organized into different modules where most of them map to a specific Emacs mode. As such installation is done per module. Go to the *Install* section of a module of interest for guidance on its configuration.
