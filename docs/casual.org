...
- Casual Dired: GNU Core Utilities 8.32+ ([[info:coreutils#Top][coreutils#Top]])

- Casual Image: [[https://imagemagick.org][ImageMagick]] 6+

- Casual CSV: [[https://elpa.gnu.org/packages/csv-mode.html][csv-mode]] 1.27+ (installed as a dependency)
...
#+VINDEX: casual-keybinding-primary
#+VINDEX: casual-keybinding-secondary

Standard installation of the ~casual~ package can be done from the following distribution sites:

- [[https://melpa.org/#/casual][MELPA]] / [[https://stable.melpa.org/#/casual][MELPA Stable]]
- [[https://elpa.nongnu.org/nongnu/casual.html][NonGNU ELPA]]

Given installation via ~package-install~ ([[info:emacs#Package Installation]]), most Casual Transient menus are autoloaded ([[info:elisp#Autoload]]) so they can be invoked without need to load the library they are defined in.
...
By default ~casual-init~ will setup keybindings for all modules ([[#casual-modules][Casual Modules]]) supported by Casual. Configure the customizable hook variable ~casual-init-hook~ to control which modules to enable.
...
Casual is designed to be configured via the Emacs customize interface. To configure Casual, invoke the following:

#+begin_example
  M-x customize-group casual
#+end_example


#+TEXINFO: @subsubheading Individual Module Install

If individual installation of a Casual module is preferred, then the *Install* section for each module is enumerated below.
...
Casual makes explicit the display of {{{kbd(C-g)}}} and {{{kbd(C-q)}}} at the bottom of its menus, which differs from the default behavior of Transient.
...
To distinguish Transient menu (prefix) definitions from existing Emacs menu definitions using keymaps ([[info:elisp#Menu Keymaps][elisp#Menu Keymaps]]), Casual uses the naming suffix  ~-tmenu~.

* Casual Modules
:PROPERTIES:
:CUSTOM_ID: casual-modules
:END:
#+CINDEX: Casual Modules
