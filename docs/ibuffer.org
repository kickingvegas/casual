...
Casual IBuffer (library: ~casual-ibuffer~) provides a user interface to Emacs IBuffer ([[info:emacs#Buffer Menus][emacs#Buffer Menus]]), a mode designed for managing buffers.
...
The primary interface for Casual IBuffer is the Transient menu ~casual-ibuffer-tmenu~. It is autoloaded ([[info:elisp#Autoload]]) so if Casual is installed via ~package-install~ ([[info:emacs#Package Installation]]) then ~casual-ibuffer-tmenu~ can be invoked via ~execute-extended-command~ ({{{kbd(M-x)}}}) without need for modifying your Emacs initialization file.
...
#+begin_src elisp :lexical no
  (require 'casual-ibuffer) ; load all dependencies including `ibuffer'
...
#+TEXINFO: @subsubheading Additional Bindings and Configuration

Like Casual Dired ([[#dired][Dired]]), it is convenient to have the menus for filtering and sorting bound as well. Listed below shows an example of binding ~casual-ibuffer-filter-tmenu~ and ~casual-ibuffer-sortby-tmenu~ to ~F~ and ~s~ respectively.
...
- I-Search/Occur Marked :: I-Search and Occur commands for marked buffers are here. Note that while you are in an I-Search session, the [[#isearch][Transient menu for I-Search]] is available to you.

- Query Replace Marked :: Commands to search & replace text in marked buffers are provided here. Note that commands in this section that modify buffers do /not/ save said buffers.

Note that the menu item “{{{kbd(RET)}}}” has "do what I mean"  (DWIM) behavior. If the point is currently on a filter group (described below) then pressing the {{{kbd(RET)}}} key will toggle the visibility of items matching that filter group. Otherwise, it will visit (open) the buffer.
...
Buffers can be /marked/ using different criteria to be operated on. Common operations include saving and deleting buffers. Note that deleting a buffer populated with a visited file is /not/ deleting the visited file. If no buffer is marked, then the buffer under the current point is operated on.

#+TEXINFO: @subheading Operations
#+VINDEX: casual-ibuffer-operations-tmenu

Pressing “{{{kbd(M)}}} More›” from the Operations section in the main menu will raise a menu (~casual-ibuffer-operations-tmenu~) providing additional commands to operate on a buffer.

[[file:images/casual-ibuffer-operations.png]]


#+TEXINFO: @subheading Mark
#+VINDEX: casual-ibuffer-mark-tmenu

Pressing “{{{kbd(t)}}} Type›” from the Operations section in the main menu will raise a menu (~casual-ibuffer-mark-tmenu~) to mark buffers by type. 


[[file:images/casual-ibuffer-mark.png]]

#+TEXINFO: @subheading Mark Regexp
#+VINDEX: casual-ibuffer-mark-regexp-tmenu

Pressing “{{{kbd(r)}}} Regexp›” from the Operations section in the main menu will raise a menu (~casual-ibuffer-mark-regexp-tmenu~) to mark buffers by regexp. 

[[file:images/casual-ibuffer-mark-regexp.png]]
...
[[file:images/casual-ibuffer-main-unicode.png]]

By enabling “{{{kbd(u)}}} Use Unicode Symbols” from the Settings menu, Casual IBuffer will use Unicode symbols as appropriate in its menus.
